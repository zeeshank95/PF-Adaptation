...
class Trainer(object):
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if distributed_utils.is_master(self.args):  # only save one checkpoint
            ...
            # for module_idx, (name, module) in enumerate(self.model.named_parameters()):
            #     if 'weight' in name and not 'norm' in name and not 'embed' in name:
            #         module.data[self.weight_0_mask[module_idx]] = self.weight_0[module_idx]  
            extra_state['train_meters'] = self.meters       

            #changes made to fairseq : to save language masks:                     
            checkpoint_utils.save_state(
                filename, self.args, self.get_model().state_dict(), self.criterion,
                self.optimizer, self.lr_scheduler, self.get_num_updates(),
                self._optim_history, extra_state, self.current_masks
            )
    ...
    def load_checkpoint(
        ...
        #changes made to fairseq : it returns state as well to extract saved masks in train.py
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        # print(self.embed_dict[self.args.dataset_name])
        # print(self.model.state_dict()['encoder.embed_tokens.weight'])
        # input('................')
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    #changes made to fairseq : extra arguments [self.current_masks, self.current_dataset_idx]
                    #passed to self.task.train_step in fairseq_task.py,                      

                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.criterion, self.optimizer, 
                        self.current_masks, self.current_dataset_idx,
                        ignore_grad
                    )
        ...
        try:
            ...
            #changes made to fairseq : When training baseline MNMT there 
            #is no pruning hence no need for making the weights 0.
            if not self.args.init_multi:
    ...
    def pruning_mask(self, weights, previous_mask, layer_idx):
            ...
            cutoff_rank = round(self.args.prune_perc * tensor.numel())
            ...
            remove_mask = weights.abs().le(cutoff_value.cuda()) * \
    ...
    #changes made to fairseq :
    def prune_model(self, previous_msk, current_idx):
        self.current_dataset_idx = current_idx
        self.previous_masks = previous_msk
        current_mask_ = {}
        for module_idx, (name, module) in enumerate(self.model.named_parameters()):
            if 'weight' in name and not 'norm' in name and not 'embed' in name:
                mask = self.pruning_mask(module.data, self.previous_masks[module_idx], module_idx)
                current_mask_[module_idx] = mask.cuda()
                weight = module.data
                weight[current_mask_[module_idx].eq(0)] = 0.0
        self.current_masks = current_mask_


    #changes made to fairseq :
    def generate_init_mask(self):
        init_mask = {}
        self.current_dataset_idx = 1
        for module_idx, (name, module) in enumerate(self.model.named_parameters()):
            if 'weight' in name and not 'norm' in name and not 'embed' in name:
                mask = torch.ByteTensor(module.data.size()).fill_(1)
                mask = mask.cuda()
                init_mask[module_idx] = mask
        self.current_masks = init_mask
        print("--------------initialsed MNMT model and generated the mask--------------")
        print("--------------Training Parent MNMT on the concatenation of all the datasets--------------")

    #changes made to fairseq :
    def mask_to_trainer(self, prev_mask, data_idx):
        print("--------------continuin training--------------")
        self.current_dataset_idx = data_idx
        self.current_masks = prev_mask

    #changes made to fairseq :
    def make_finetuning_mask(self, previous_msk, current_idx):
        ...
        print("-------------- Training language specific parameters for {} --------------".format(self.args.dataset_name))
    ...
    # def make_weights_0(self):
    #     self.weight_0 = {}
    #     self.weight_0_mask = {}
    #     for module_idx, (name, module) in enumerate(self.model.named_parameters()):
    #         if 'weight' in name and not 'norm' in name and not 'embed' in name:
    #             layer_mask = self.current_masks[module_idx].clone()
    #             mask_0_gt = layer_mask.gt(1)
    #             mask_0_lt = layer_mask.lt(self.current_dataset_idx)
    #             mask_0 = mask_0_gt*mask_0_lt
    #             mask_0_zero = layer_mask.eq(0)
    #             mask_0 = mask_0+mask_0_zero
                
    #             layer_mask[mask_0] = 1
    #             layer_mask[~mask_0] = 0
    #             self.weight_0_mask[module_idx] = layer_mask
    #             self.weight_0[module_idx] = module.data[layer_mask]
    #             module.data[layer_mask.eq(1)] = 0.0